...
This module extends the functionality of the original ``main.py`` sample
and produces a more comprehensive set of mental‑math practice sheets for
teenagers.  The original generator created random arithmetic slips with
little control over the distribution of problem types.  Here we provide
two distinct generators:

1. ``ArithmeticSlipGenerator`` – Generates slips of short arithmetic
   questions covering addition, subtraction, multiplication, division,
   ...
   cubes from 1–10 accelerates time‑consuming computations【432673483560871†L80-L84】.

2. ``EquationSlipGenerator`` – Generates slips of simple algebraic
...
The ``SlipPDFBuilder`` class ties everything together and writes the
questions and optionally the answers to PDF files.  It produces a
2×5 grid of slips per page on an A4 sheet, similar to the original
reference.  Each slip has space below each question for the student
to write their answer.

Example usage (see ``__main__`` for more detail):
...
"""This module uses Matplotlib to generate PDFs when ReportLab is unavailable.

Because the execution environment does not include the ``reportlab`` package
required by the original generator, we fall back to the widely available
``matplotlib`` library.  Each slip page and answer page is drawn on an A4
canvas using matplotlib primitives and saved via ``PdfPages``.  If
ReportLab becomes available in the future the original implementation
